    ...
    @if(!empty($existing_attachments))
                ...
                <div class="group relative bg-gray-100 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-zinc-700 overflow-hidden transition-all duration-200 hover:shadow-md">
                    @if($isImage)
                        <img
                            ...
                            @click.stop.prevent="$dispatch('open-modal', { id: 'image-preview-{{ $attachment['attachment_id'] }}' })"
                        />
                    @else
        ...
        @foreach($existing_attachments as $attachment)
            ...
            @if(in_array(strtolower(pathinfo($attachment['file_name'], PATHINFO_EXTENSION)), ['jpg','jpeg','png','gif','webp']))
                <div x-data="{ open: false }" @open-modal.window="if($event.detail.id === 'image-preview-{{ $attachment['attachment_id'] }}') open = true">
                    <div
                        x-show="open"
                    ...
                    >
                        <div x-transition.scale class="relative max-w-4xl w-[90%]">
                            <button @click="open = false"
                                    class="absolute top-3 right-3 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 transition">
                                <x-heroicon-o-x-mark class="w-5 h-5" />
                            </button>
                            <img src="{{ $url }}" alt="{{ $attachment['file_name'] }}"
                                class="rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 max-h-[85vh] w-full object-contain" />
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div
                ...
                x-on:open-delete-attachment-modal-{{ $attachment['attachment_id'] }}.window="showModal = true"
                x-on:close-delete-attachment-modal-{{ $attachment['attachment_id'] }}.window="showModal = false"
            ...
            >
                ...
                <div x-show="showModal" x-transition.scale
                    ...
                    <div wire:loading.remove wire:target="removeAttachment({{ $attachment['attachment_id'] }})">
